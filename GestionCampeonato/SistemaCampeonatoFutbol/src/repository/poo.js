...
class Torneo {
  ...
  generarEnfrentamientos() {
    ...
    const intervaloEnfrentamientos = 1; // Define el intervalo de tiempo entre enfrentamientos
  ...
  registrarResultados() {
    ...
    golesEquipoAInputs.forEach((input, index) => {
      ...
      // Realiza el registro de resultados y demás datos
    ...
    // Almacena los partidos jugados en el almacenamiento local
    localStorage.setItem("partidosJugados", JSON.stringify(partidosJugados));

    // Calcula y muestra la tabla de posiciones
    this.calcularYMostrarTablaPosiciones();

    // Cierra el modal después de registrar los resultados
    const modal = new bootstrap.Modal(
      document.getElementById("enfrentamientoModal")
    );
    modal.hide();
